...
public class WorkflowInstancesDetailedGet extends AbstractWorkflowWebscript
{
    ...
    protected Map<String, Object> buildModel(WorkflowModelBuilder modelBuilder, WebScriptRequest req, Status status, Cache cache)
    {
        ...
        if (workflowDefinitionId == null)
        {
            ...
            // FIXME: WA for api/alvex/workflow-instances/?includeTasks=true being redirected here instead on correct API
            if (workflowDefinitionId == null) {
                ...
                if( pos >= skipCount && pos < endPoint )
                {
                    ...
                    for (WorkflowTask task : tasks)
                    {
						...
						if( shortName != null )
						{
							...
							NodeRef person = personService.getPersonOrNull( shortName );
