 ...
 */
(function($) {
    var baseComponent = new Alfresco.component.Base;
    window.msg = baseComponent.msg;
    ...
    var months = [window.msg("month.january"), window.msg("month.february"), window.msg("month.march"), window.msg("month.april"),
        window.msg("month.may"), window.msg("month.june"), window.msg("month.july"), window.msg("month.august"),
        window.msg("month.september"), window.msg("month.october"), window.msg("month.november"), window.msg("month.december")];
    ...
    var str_opt_dow = "";
    var days = [window.msg("day.sunday"), window.msg("day.monday"), window.msg("day.tuesday"), window.msg("day.wednesday"),
        window.msg("day.thursday"), window.msg("day.friday"), window.msg("day.saturday")];
    ...
    for (var i = 0; i < periods.length; i++) {
        str_opt_period += "<option value='"+periods[i] + "'>" + window.msg("cron." + periods[i]) + "\n";
    }
    ...
    function getCronType(full_cron_str, opts) {
        // if customValues defined, check for matches there first
        var cron_str = full_cron_str.replace(" false", "").replace(" true", "");
    ...
    function getCurrentValue(c) {
        ...
        var repeat = false;
        if($(c).find("input.cron-repeat-checkbox")[0].checked){
            repeat = true;
        }
        ...
        return [min, hour, day, month, dow, repeat].join(" ");
    }
    ...
    var methods = {
        init : function(opts) {
            ...
            block["period"] = $("<span class='cron-period'>"
                + window.msg('cron.period') + " <select name='cron-period'>" + custom_periods
            ...
            block["dom"] = $("<span class='cron-block cron-block-dom'>"
                + window.msg("cron.day") + " <select name='cron-dom'>" + str_opt_dom
                + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["month"] = $("<span class='cron-block cron-block-month'>"
                + window.msg("cron.month") + " <select name='cron-month'>" + str_opt_month
                + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["mins"] = $("<span class='cron-block cron-block-mins'>"
                + window.msg("cron.mins") + " <select name='cron-mins'>" + str_opt_mih
            ...
            block["dow"] = $("<span class='cron-block cron-block-dow'>"
                + window.msg("cron.day") + " <select name='cron-dow'>" + str_opt_dow
                + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["time"] = $("<span class='cron-block cron-block-time'>"
                + window.msg("cron.hour") + " <select name='cron-time-hour' class='cron-time-hour'>" + str_opt_hid
        ...
        value : function(cron_str, id) {
            ...
            var repeat = (d[5] || false) ;

            $(this).find("input.cron-repeat-checkbox").prop('checked',  (repeat == 'true'));
